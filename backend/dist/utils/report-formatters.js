...
class ReportFormatter {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                for (let i = 0; i < pageCount; i++) {
                    doc.switchToPage(i);
                    doc.font('Times-Roman').fontSize(10).text(encodeText(`Страница ${i + 1} из ${pageCount}`), 50, doc.page.height - 50, { align: 'center' });
                }
                doc.end();
            }
            catch (error) {
    ...
    static async toCSV(report) {
        ...
        const csvContent = await fs_1.promises.readFile('temp.csv', 'utf8');
        await fs_1.promises.unlink('temp.csv');
        return csvContent;
    }
}
